...
import numpy as np
from pyrtid.inverse.regularization import CovarianceMatrixbyUd
from pyrtid.utils import NDArrayFloat
from scipy._lib._util import check_random_state  # To handle random_state
from scipy.sparse.linalg import (  # IterativeSolve
    LinearOperator,  # Matrix-free IterativeSolve
    eigsh,
    gmres,
    minres,
    svds,
...
class PCGA:
    ...
    def __init__(
        ...
        s_init: NDArrayFloat,
        Q: CovarianceMatrixbyUd,
        pts: NDArrayFloat,
        params,
        s_true: bool = None,
        obs: Optional[NDArrayFloat] = None,
        obs_true=None,
        ...
        s_init : numpy.ndarray
            m by 1 initial solution for Gauss-Newton method. In theory, the choice of
        ...
        self.s_init = self.s_init.reshape(
            -1, 1
        ...
        self.obs_true = np.array(
            obs_true
        )  # observation without noise for synthetic problems
        ...
        self.iter_save = (
            False if "iter_save" not in self.params else self.params["iter_save"]
        )
        ...
        if "R" in params:
            ...
            if self.R.ndim == 0:
                # convert to 1d array (1,)
                self.R = np.array(params["R"]).reshape(-1)
            elif self.R.ndim == 1:
                self.R = np.array(params["R"]).reshape(
                    -1, 1
                )  # convert to 2d n x 1 array
            elif self.R.ndim == 2:
                self.R = np.array(params["R"]).reshape(
                    -1, 1
                )  # convert to 2d n x 1 array
            ...
            self.invsqrtR = 1.0 / np.sqrt(self.R)
            self.invR = 1.0 / self.R
        else:
            raise ValueError("You should provide R")

        # posterior cov computation
        # we only support the computation of post variance (diagonals of
        # posterior covariance matrix) for now
        # Eigenvalues and eigenvectors of the posterir covariance matrix
        # - will be implemented later
        # self.postU = None
        # self.postd = None

        # will be updated if post_cov == "diag"
        self.post_diagv = self.prior_std**2
        ...
        if self.post_cov or self.post_cov == "diag":
            ...
            if "post_diag_direct" in params:
                ...
                if self.post_diag_direct:
                    print(
                        "WARNING!! : you chose to perform direct posterior variance "
                        "analysis, which would take forever! You can use "
                        "post_diag_direct=False"
                    )
        ...
        if "LM" in params:
            ...
            if "alphamax_LM" in params:
                ...
                self.alphamax_LM = 10.0**3.0  # does it sound ok?
        ...
        self.linesearch = (
            True if "linesearch" not in self.params else self.params["linesearch"]
        )

        if self.linesearch:
            self.nopts_LS = (
                self.ncores
                if "nopts_LS" not in self.params
                else self.params["nopts_LS"]
            )

        self.maxiter = (
            10 if "maxiter" not in self.params else self.params["maxiter"]
        )  # maximum iteration
        self.restol = (
            1e-2 if "restol" not in self.params else self.params["restol"]
        )  # rel tol

        # PCGA parameters (purturbation size)
        self.precision = (
            1.0e-8 if "precision" not in self.params else self.params["precision"]
        )

        # Printing verbose
        self.verbose = False if "verbose" not in self.params else self.params["verbose"]
        self.forward_model_verbose = (
            True
            if "forward_model_verbose" not in self.params
            else self.params["forward_model_verbose"]
        )
        ...
        print(
            "   Prior scale (correlation) parameter              : %s"
            % (self.prior_cov_scale)
        )
        print(
            "   Posterior cov computation                        : %s" % (self.post_cov)
        )
        if self.post_cov:
            if self.post_diag_direct:
        ...
        print(
            "   Number of CPU cores (n_core)                     : %d" % (self.ncores)
        )
        print(
            "   Maximum GN iterations                            : %d" % (self.maxiter)
        )
        print(
            "   machine precision (delta = sqrt(precision))      : %e"
            % (self.precision)
        )
        print(
            "   Tol for iterations (norm(sol_diff)/norm(sol))    : %e" % (self.restol)
        )
        print("   Levenberg-Marquardt (LM)                         : %s" % (self.LM))
        if self.LM:
            print(
                "   LM solution range constraints (LM_smin, LM_smax) : %s, %s"
                % (self.LM_smin, self.LM_smax)
            )
        ...
        if self.parallel:
            ...
            for i in range(nruns):
                if i == 0:
                    ...
                    simul_obs_purturbation = np.concatenate(
                        (simul_obs_purturbation, self.ForwardSolve(x[:, i : i + 1])),
                        axis=1,
                    )
    ...
    def CreateSyntheticData(self, s=None, noise=False):
        ...
        if s is None:
            if s_true is None:
                ...
                print(
                    "- generate noisy observations for this synthetic inversion problem"
                )
    ...
    def ObjectiveFunctionNoBeta(self, s_cur, simul_obs, approx=True):
        ...
        if approx:
            ...
            if p == 1:
                ...
                # inexpensive solve p by p where p <= 3, usually p = 1 (scalar division)
                tmp = np.linalg.solve(XTinvQX, XTinvQs)
    ...
    def DirectSolve(self, s_cur, simul_obs=None):
        ...
        for i in range(n_pc):
            Z[:, i : i + 1] = np.dot(
                sqrt(self.priord[i]), self.priorU[:, i : i + 1]
        ...
        # sqrtGDCov = GeneralizedSQRTCovarianceMatrix(U_data,HZ)
        # # sqrt of generalized data covariance
        # sigma_cR = svds(sqrtGDCov, k= min(n-p,n_pc), which='LM', maxiter = n,
        ...
        for i in range(nopts):  # sequential evaluation for now
            ...
            s_hat_all[:, i : i + 1] = np.dot(self.X, beta_all[:, i : i + 1]) + np.dot(
                HQ.T, xi
            )
            ...
            if self.parallel:
                ...
                for i in range(nopts):
                    if i == 0:
                        ...
                        if LM_eval[i]:
                            simul_obs_all = np.concatenate(
                                (simul_obs_all, -10000.0 * np.ones(simul_obs.shape)),
                                axis=1,
                            )
                        else:
                            simul_obs_all = np.concatenate(
                                (
                                    simul_obs_all,
                                    self.ForwardSolve(s_hat_all[:, i : i + 1]),
                                ),
                                axis=1,
                            )
        else:
    ...
    def IterativeSolve(self, s_cur, simul_obs=None, precond=False):
        ...
        for i in range(nopts):  # this is sequential for now
            ...
            if self.precond:
                ...
                restart = (
                    50
                    if "gmresrestart" not in self.params
                    else self.params["gmresrestart"]
                )
                x, info = gmres(
                    Afun,
                    b,
        ...
        # savemat('Q2cR.mat',{'X':self.X,'HZ':HZ,'HX':HX,'b':b,'alpha':alpha,
        # 'i_best':i_best,'R':self.R,'pts':self.pts,'Z':Z})

        self.i_best = i_best  # keep track of best LM solution
        return s_hat, beta, simul_obs_new

    def LinearInversionKnownMean(self, s_cur, beta=0.0):
        # will be implemented later.
        raise NotImplementedError
    ...
    def LinearIteration(self, s_cur, simul_obs):
        ...
        if direct:
            ...
            s_hat, beta, simul_obs_new = self.IterativeSolve(
                s_cur, simul_obs, precond=precond
            )
    ...
    def LineSearch(self, s_cur, s_past):
        ...
        for i in range(nopts):
            if self.objeval:  # If true, we do accurate computation
                obj = self.ObjectiveFunctionNoBeta(
                    s_hat_all[:, i : i + 1], simul_obs_all[:, i : i + 1], 0
                )
            else:  # we compute through PCGA approximation
                obj = self.ObjectiveFunctionNoBeta(
                    s_hat_all[:, i : i + 1], simul_obs_all[:, i : i + 1], 1
                )
    ...
    def GaussNewton(self):
        ...
        nRMSE_init = np.linalg.norm(
            np.divide(simul_obs_init - self.obs, self.sqrtR)
        ) / np.sqrt(n)
        print(
            "obs. RMSE (norm(obs. diff.)/sqrt(nobs)): %g, normalized obs. RMSE "
            "(norm(obs. diff./sqrtR)/sqrt(nobs)): %g" % (RMSE_init, nRMSE_init)
        )
        ...
        for i in range(maxiter):
            ...
            s_cur, beta_cur, simul_obs_cur, obj = self.LinearIteration(
                s_past, simul_obs
            )
            ...
            if self.s_true is not None:
                ...
                print(
                    "= L2-norm error (w.r.t truth) is %g, obs. RMSE is %g, "
                    "obs. normalized RMSE is %g" % (err, RMSE_cur, nRMSE_cur)
                )
            else:
                print("== iteration %d summary ==" % (i + 1))
                print(
                ...
                print(
                    "= obs. RMSE is %g, obs. normalized RMSE is %g"
                    % (RMSE_cur, nRMSE_cur)
                )
            ...
            if self.iter_save:
                ...
                # self.post_diagv = self.ComputePosteriorDiagonalEntries(self.HZ,
                # self.HX, self.i_best,self.R)
                # if self.post_cov or self.post_cov == "diag":
        ...
        if (
            self.post_cov or self.post_cov == "diag"
            ...
            if self.post_diag_direct:
                ...
                self.post_diagv = self.ComputePosteriorDiagonalEntriesDirect(
                    self.HZ, self.HX, self.i_best, self.R
                )
            else:
        ...
        print(
            ...
            % (
                np.linalg.norm(self.simul_obs_best - self.obs) / np.sqrt(self.n),
                RMSE_init,
                np.linalg.norm(np.divide(self.simul_obs_best - self.obs, self.sqrtR))
                / np.sqrt(n),
                nRMSE_init,
            )
        )
    ...
    def ComputePosteriorDiagonalEntries(self, HZ, HX, i_best, R):
        ...
        if R.shape[0] == 1:
            ...
            def invPsi(v):
                ...
                if Psi_UTv.ndim == 1:
                    PsiDPsiTv = np.dot(
                        Psi_U,
                        np.multiply(
                            Dvec[: Psi_U.shape[1]].reshape(Psi_UTv.shape), Psi_UTv
                        ),
                    )
                elif Psi_UTv.ndim == 2:  # for invPsi(HX)
                    DMat = np.tile(
            ...
            def invPsi(v):
                ...
                if Psi_UTv.ndim == 1:
                    ...
                    # may need to change this later in a more general way 06142018 Harry
                    RMat = np.tile(
                    ...
                    raise ValueError(
                        "Psi_U times vector should have a dimension smaller than 2"
                        "- current dim = %d" % (Psi_UTv.ndim)
                    )

                return alphainvRv - PsiDPsiTv
                # return np.multiply(np.multiply((1. / alpha[i_best]),
                # self.invR.reshape(v.shape)), v)
                # - np.dot(Psi_U,np.multiply(Dvec[:Psi_U.shape[1]]
                # .reshape(Psi_UTv.shape), Psi_UTv))
        ...
        for i in range(m):
            b = np.zeros((n + p, 1), dtype="d")
            b[0:n] = np.dot(
                HZ, (np.multiply(np.sqrt(self.priord), self.priorU[i : i + 1, :].T))
            )
            b[n : n + p] = self.X[i : i + 1, :].T
